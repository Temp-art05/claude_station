...
| **Chat** | Agent SDK sessions in the selected repo: streaming, tool-approval modal with countdown, interrupt, resume across restarts, per-message history in our own DB, paste screenshots. |
...
| **Jira** | My-issues list + JQL, ADF→markdown detail, comment / transition / log-work from the UI, "Work on this with Claude" seeds a session with the issue as context. |
| **GitHub** | PRs and issues via the `gh` CLI (your existing login), same "Work with Claude" entry point. |
