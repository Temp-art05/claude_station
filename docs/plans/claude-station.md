...
- **rev15 (2026-08-14)** — Fix checkout branch có `/` + gom branch theo folder (plan: `branch-menu-nested-branches.md`). **Bug**: `checkout()` đoán remote ref bằng hình dạng tên (`/^[^/]+\/(.+)$/`) nên click branch local `version/4.0.0` chạy `git switch -c 4.0.0 --track version/4.0.0` → **tạo branch mới `4.0.0`** thay vì checkout (đúng với mọi `feature/x`, `release/1.2`…). Fix: tra ref thật — `refs/heads/<name>` trước (local thắng shorthand remote, giống `git switch`), rồi `refs/remotes/<name>` (cắt prefix bằng danh sách `git remote`, không cắt segment đầu), không khớp thì để git tự báo lỗi. UI: `shared/branch-tree.ts` `buildBranchTree` gom prefix thành folder (chỉ khi ≥2 branch cùng prefix — tránh folder 1 con), BranchMenu render tree collapse sẵn (mở sẵn folder chứa branch current, search auto-expand hết, Esc clear search rồi mới đóng popup); leaf hiển thị segment cuối nhưng **checkout luôn gửi full ref**. Test: `git-checkout.test.ts` (repo git thật + clone) khoá regression này.
